...
function getDashPeriodAds(prodFilter=null){
  const now=new Date(), ym=now.toISOString().slice(0,7), yr=String(now.getFullYear());
  let ads=S.db.ads;
  if(S.dashPeriod==='custom'&&S.customRanges.dash){
    const {start,end}=S.customRanges.dash;
  ...
  if(prodFilter) ads=ads.filter(a=>a.product_name===prodFilter);
  return ads;
}
function renderDash(){
  ...
  const adsBudget=getDashPeriodAds(prodFilter).reduce((a,x)=>a+(x.budget||0),0);
  ...
  draw7Days(filteredSales); drawChannel(filteredSales,filteredFin,prodFilter);
}
...
function drawChannel(filteredSales,filteredFin,prodFilter=null){
